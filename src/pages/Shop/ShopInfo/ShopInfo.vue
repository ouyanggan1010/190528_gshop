...
<script>
	...
	export default {
		...
		//不能用watch因为数据在它的父组件已经获取，所以watch不会执行
		//数据获取实在前一步就获取到了；挂载html页面，创建组件  执行的mounted
		mounted(){//从别的组件切换过来，数据已经加载；如果是在当前组件刷新更新，数据还没有加载进来，因为是在别的地方请求的
			//判断数据是否有，没有的话直接结束
		...
		methods:{
			_initScroll(){
				...
				picList.style.width = (count * lis - 6)+'px';
		...
		watch:{
			info(){//从别的组件切换进来，数据已经加载过了，watch不会执行；在当前组件刷新更新，数据加载比较慢，可以监听到watch变化
				this._initScroll();
			}
		}
	}
